...
// ref: https://github.com/apple/swift-syntax/blob/main/Examples/Sources/MacroExamples/Implementation/Member/CustomCodable.swift

public enum CustomCodingKeys: MemberMacro {
    public static func expansion(
        ...
        let cases = memberList.compactMap({ member -> String? in
            ...
            if let customKeyMacro = member.decl.as(VariableDeclSyntax.self)?.attributes.first(where: { element in
                element.as(AttributeSyntax.self)?.attributeName.as(IdentifierTypeSyntax.self)?.description == "CustomCodingKey"
            }) {
...
public enum CustomCodingKey: PeerMacro {
    public static func expansion(
        of node: AttributeSyntax,
